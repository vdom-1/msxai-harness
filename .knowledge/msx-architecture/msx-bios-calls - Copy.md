...
Function: Tests whether the character of (`HL`) is the specified character if not, it generates SYNTAX ERROR, otherwise it goes to [CHRGTR](#chrgtr) (`#0010`)
...
Output: `HL` is increased by one and `A` receives (`HL`), When the tested character is numerical, the carry flag is set the end of the statement (`00h` or `3Ah`) causes the zero flag to be set
...
```
A:
  7: Expanded slot (0 = no, 1 = yes)
  6-4: Unused
  3-2: Secondary slot number (00-11)
  1-0: Primary slot number (00-11)
HL: Address to read
```
...
Flags:
  C: Set if the character is a number
  Z: Set if the end of the statement has been reached
```
...
Input:
```
IY: High byte with slot ID
IX: The address that will be called
```

Remark: Variables can never be given in alternative registers or IX and IY. See [RDSLT](#rdslt)
...
Function: Returns the type of [DAC](#dac) (decimal accumulator)

Input: none ([VALTYP](#valtyp) contains the [DAC](#dac) type)
...
Registers: `AF`
Remark: Types can be recognised by the flag marked by “*”.
...
Function: Initializes the device
...
Input:  
Foreground color in [FORCLR](system-variables.md#forclr)  
Background color in [BAKCLR](system-variables.md#bakclr)  
Border color in [BDRCLR](system-variables.md#bdrclr)
...
Input: [SCRMOD](system-variables.md#scrmod)
...
Input:  
[TXTNAM](system-variables.md#txtnam)  
[TXTCGP](system-variables.md#txtcgp)
...
Input:  
[T32NAM](system-variables.md#t32nam)  
[T32CGP](system-variables.md#t32cgp)  
[T32COL](system-variables.md#t32col)  
[T32ATR](system-variables.md#t32atr)  
[T32PAT](system-variables.md#t32pat)
...
Input:  
[GRPNAM](system-variables.md#grpnam)  
[GRPCGP](system-variables.md#grpcgp)  
[GRPCOL](system-variables.md#grpcol)  
[GRPATR](system-variables.md#grpatr)  
[GRPPAT](system-variables.md#grppat)
...
Input:  
[MLTNAM](system-variables.md#mltnam)  
[MLTCGP](system-variables.md#mltcgp)  
[MLTCOL](system-variables.md#mltcol)  
[MLTATR](system-variables.md#mltatr)  
[MLTPAT](system-variables.md#mltpat)
...
Input: See [INIT32](https://map.grauw.nl/resources/msxbios.php#INIT32)
Registers: All
...
Input: See [INIGRP](https://map.grauw.nl/resources/msxbios.php#INIGRP)
Registers: All
...
Input: See [INIMLT](https://map.grauw.nl/resources/msxbios.php#INIMLT)
Registers: All
...
Address: #0090
Function: Initializes PSG and sets initial value for the PLAY statement
Registers: All
...
Address: #00B1
Function: Same as [PINLIN](https://map.grauw.nl/resources/msxbios.php#PINLIN) except that AUGFLG (#F6AA) is set
Output: HL - For the starting address of the buffer -1
           Carry flag set when it ends with the STOP key
Registers: All
...
Address: #00BD
Function: Same as [ISCNTC](https://map.grauw.nl/resources/msxbios.php#ISCNTC). used in Basic
...
Remark: On MSX2, function call numbers 8-23 are forwarded to
           [NEWPAD](https://map.grauw.nl/resources/subrom.php#NEWPAD) in the SubROM.
...
Address: #0105
Function: Tests whether [UPC](https://map.grauw.nl/resources/msxbios.php#UPC) is possible, if possible, execute [UPC](https://map.grauw.nl/resources/msxbios.php#UPC)
...
Address: #010B
Function: Tests whether [DOWNC](https://map.grauw.nl/resources/msxbios.php#DOWNC) is possible, if possible, execute [DOWNC](https://map.grauw.nl/resources/msxbios.php#DOWNC)
Output: Carry flag set if operation would end outside the screen
Registers: AF
...
Input: A  - Value to write, see [RSLREG](https://map.grauw.nl/resources/msxbios.php#RSLREG)
...
Address: #0147
Function: Initializes mass-storage media like formatting of disks
Registers: All
...
Registers: F
Remark: Differences with [LPTOUT](https://map.grauw.nl/resources/msxbios.php#LPTOUT):
...
Remark: Use of [EXTROM](https://map.grauw.nl/resources/msxbios.php#EXTROM) or [CALSLT](https://map.grauw.nl/resources/msxbios.php#CALSLT) is more convenient.
...
Address: #016B
Function: Same function as [FILVRM](https://map.grauw.nl/resources/msxbios.php#FILVRM) (with 16-bit VRAM-address).
Input: HL - Address
           BC - Length
           A  - Data
Registers: AF,BC
...
```
A: value to write
When bit 7 is reset it shows the MSX 2+ start-up screen on boot, and counts and initializes the RAM.
```
...
```
A:  
  7: Indicates whether the Turbo LED is switched with the CPU 
  6-2: unused/fixed 0
  1-0:  
    00: Z80 (ROM mode)  
    01: R800 ROM mode  
